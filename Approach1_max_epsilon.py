'''
Approach1_max_epsilon
'''
import gurobipy as grb
import numpy as np

def Approach1_max_epsilon(x, A, AC, q, best_gamma, epsilon0, s, G):

    Model = grb.Model("Approach1_max_epsilon")
    ...
    gamma = Model.addVars(range(1, m + 1), range(1, s), lb=0, vtype=grb.GRB.CONTINUOUS, name='gamma')
    gg1 = Model.addVars(range(1, m + 1), range(1, s), lb=-10, vtype=grb.GRB.CONTINUOUS, name='gg1')
    ...
    Model.addConstrs(global_u[A[i - 1]] <= b[AC[i - 1]] - epsilon for i in range(1, len(A) + 1) if
                     AC[i - 1] != 1 and AC[i - 1] != q)
    Model.addConstrs(
        global_u[A[i - 1]] <= b[AC[i - 1]] - epsilon for i in range(1, len(A) + 1) if AC[i - 1] == 1)
    Model.addConstrs(global_u[A[i - 1]] >= b[AC[i - 1] - 1] for i in range(1, len(A) + 1) if AC[i - 1] == q)

    Model.addConstr(epsilon >= epsilon0)

    Model.addConstr(best_gamma == grb.quicksum(gamma[j, l] for j in range(1, m + 1) for l in range(1, s)))

    Model.addConstrs(gg1[j, l] == v[j, l + 1]/(G[j - 1, l + 1] - G[j - 1, l]) - v[j, l]/(G[j - 1, l] - G[j - 1, l - 1]) for j in range(1, m + 1) for l in range(1, s))
    Model.addConstrs(gamma[j, l] == grb.abs_(gg1[j, l]) for j in range(1, m + 1) for l in range(1, s))
