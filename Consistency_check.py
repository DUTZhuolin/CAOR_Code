...
def Consistency_check(x, A, AC, q, epsilon, s, G):

    Model = grb.Model("Consistency_check")
    ...
    e1 = Model.addVars(range(1, len(A) + 1), lb=0, vtype=grb.GRB.CONTINUOUS, name='e1')
    e2 = Model.addVars(range(1, len(A) + 1), lb=0, vtype=grb.GRB.CONTINUOUS, name='e2')

    Model.update()

    Model.setObjective(grb.quicksum(e1[i] + e2[i] for i in range(1, len(A)+1)), grb.GRB.MINIMIZE)
    ...
    Model.addConstrs(global_u[A[i - 1]] >= b[AC[i - 1] - 1] - e1[i] for i in range(1, len(A) + 1) if
                     AC[i - 1] != 1 and AC[i - 1] != q)
    Model.addConstrs(global_u[A[i - 1]] <= b[AC[i - 1]] + e2[i] - epsilon for i in range(1, len(A) + 1) if
                     AC[i - 1] != 1 and AC[i - 1] != q)

    Model.addConstrs(global_u[A[i - 1]] <= b[AC[i - 1]] + e2[i] - epsilon for i in range(1, len(A) + 1) if AC[i - 1] == 1)
    Model.addConstrs(global_u[A[i - 1]] >= b[AC[i - 1] - 1] - e1[i] for i in range(1, len(A) + 1) if AC[i - 1] == q)
